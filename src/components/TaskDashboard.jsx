  ...
  return (
    <div className="min-h-screen bg-gray-100 p-4 sm:p-6 lg:p-8">
      ...
      <div className="rounded-lg lg:shadow overflow-hidden">
        ...
        {/* Mobile View */}
        <div className="mb-6 flex flex-col sm:flex-column sm:justify-between sm:items-center gap-4 block lg:hidden">
          <h1 className="text-5xl font-bold text-gray-800 w-full">Task Dashboard</h1>
          <button
            onClick={() => setIsModalOpen(true)}
        ...
        <div className="block lg:hidden bg-white border border-gray-200 shadow-lg rounded-lg">
          {tasks.map((task, index) => (
            ...
            >
              <div className="flex justify-between items-start mb-2 bg-red">
                <div>
                  <span className={`text-2xl text-gray-500`}>Task #{index + 1}</span>
                  <h3 className={`mt-2 text-4xl  ${task.status === 'Completed' ? 'font-medium line-through text-gray-400' : 'font-bold text-gray-900 '}`}>{task.name}</h3>
                </div>
                <button
                  onClick={() => handleDelete(task.id)}
